...
avg_score_components_by_section = np.zeros((sections, 5))
for i in range(sections):
    for j in range(num_score_components):
        index = i*num_score_components + j
        avg_score_components_by_section[i, j] = np.mean(all_data[:, index], axis=0)
    #avg_score_components_by_section[i] = np.mean(all_data_reshaped[:, :, i], axis=0)



fig, axes = plt.subplots(nrows=7, ncols=2, figsize=(8,5))
loc = ticker.MultipleLocator(base=0.2) # this locator puts ticks at regular intervals

index = np.arange(len(x_labels)) + 0.3

axes[0,0].bar(x=index, height=avg_score_components_by_section[0], edgecolor='#000000') # , color=colors)
...
axes[0,0].set_ylim(0, 1.5*np.max(avg_score_components_by_section[0]))
axes[0,0].yaxis.set_major_locator(loc)
axes[0,0].title.set_text('SGConvLSTM')
