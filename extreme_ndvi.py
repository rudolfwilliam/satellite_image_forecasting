...
def main(index=0):
    ...
    files = os.listdir('ndvi_pickles')
    no_files = len(files)

    with open('ndvi_pickles/'+files[0], 'rb') as inp:
    ...
    for i in range(no_files):
        with open('ndvi_pickles/'+files[i], 'rb') as inp:
            data = pickle.load(inp)
        q_t += data[1]
        cur_q_ps = data[3]
    ...
    #labels = ['Feb', 'March', 'April', 'May', 'June', 'July', 'Aug', 'Sep', 'Oct', 'Nov']
